...
def _update_hosts_for_group_changes(host_id_list: list[str], group_id_list: list[str], identity: Identity):
    ...
    for hostId in host_id_list:
        client.ReportHostMoved(hostId, group_id) # Update groups data on each host record, current a network call to Kessel but will be writing to an outbox table (can probably be batched with the other SQL command rather than a loop)
    db.session.commit()
